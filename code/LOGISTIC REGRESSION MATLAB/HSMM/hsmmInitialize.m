%% REPLACED 
function [A,B,P,PAI,Vk,MO,K, lambdas]=hsmmInitialize(MO,M,D,K,MT, geometric, geometric_mode, init)
...
elseif strcmp(init,'rand')
    lambdas = sort(rand*mean(Vk1));%;sort(rand(1,M)*max(Vk)); %inicialização "inteligente": lambdas espaçados
else
...
A = zeros(M, M,MT(1));
for t=1:MT(1)
    A(:,:,t) = generate_random_transition_matrix_d(M);
end
    ...
    %if strcmp(geometric_mode, 'rand')
        ...
        if D~=0
            while(dmax>D)
                ...
                %geometric dist: P(D>dmax)=1-P(D<=dmax)=1-(1-(1-p)^dmax) = 0.001
                %dmax=log(0.001)/log(1-p).
