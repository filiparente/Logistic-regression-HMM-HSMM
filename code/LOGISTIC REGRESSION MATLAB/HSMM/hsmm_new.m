...
function [lambdas, lambdas_tt, PAI,A,B,P,Qest,  store_GAMMA, store_ALPHA, lkh, ll, ir]=hsmm_new(lambdas, PAI,A,B,P,MO,IterationNo,MT, tolerance, Vk)
...
%dmax = floor(log(0.01)/log(1-min(PM)));
...
store_ALPHA = zeros(M,T,D);
store_ALPHAT = zeros(M,T,D);
lkh=zeros(1,N);
ll = [];
lambdas_tt = [];
